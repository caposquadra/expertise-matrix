...
class TestExportCsv:
    async def test_csv_export(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/export/csv", headers={"Authorization": f"Bearer {manager_token}"}
        )
    ...
    async def test_csv_employee_forbidden(self, client: AsyncClient, employee_token):
        r = await client.get(
            "/api/v1/export/csv", headers={"Authorization": f"Bearer {employee_token}"}
        )
...
class TestExportExcel:
    async def test_excel_export(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/export/excel", headers={"Authorization": f"Bearer {manager_token}"}
        )
        assert r.status_code == 200
