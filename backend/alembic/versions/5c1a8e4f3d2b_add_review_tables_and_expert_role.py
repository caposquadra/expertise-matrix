...
def upgrade() -> None:
    ...
    op.create_table('review_cycles',
        ...
        sa.Column('status', sa.Enum('draft', 'manager_review', 'interview', 'decision', 'completed', 'rejected', name='review_status'), nullable=False),
        ...
        sa.Column('final_decision', sa.Enum('promoted', 'rejected', name='review_decision'), nullable=True),
        ...
        sa.ForeignKeyConstraint(['expert_id'], ['employees.id'], ),
    ...
    op.create_table('review_assessments',
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('review_cycle_id', 'skill_id', name='uq_review_cycle_skill')
    )
    op.create_index('ix_review_assessments_review_cycle_id', 'review_assessments', ['review_cycle_id'])
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE employees ALTER COLUMN role TYPE user_role USING role::text::user_role")
