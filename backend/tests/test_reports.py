...
class TestGrowthZones:
    ...
    async def test_growth_zones_limited(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/reports/growth-zones?limit=3",
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        assert r.status_code == 200
        assert len(r.json()) <= 3
...
class TestComparison:
    async def test_comparison_needs_two_ids(
        self, client: AsyncClient, manager_token, employee_user, manager_user
    ):
        r = await client.get(
    ...
    async def test_comparison_fails_with_one_id(
        self, client: AsyncClient, manager_token, employee_user
    ):
        r = await client.get(
    ...
    async def test_employee_cannot_access(
...
class TestReviewCyclesSummary:
    async def test_summary(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/reports/review-cycles-summary",
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        assert r.status_code == 200
        data = r.json()
...
class TestWeakestSkills:
    ...
    async def test_weakest_skills_limited(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/reports/weakest-skills?limit=3",
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        assert r.status_code == 200
        assert len(r.json()) <= 3
...
class TestRecentDecisions:
    async def test_recent_decisions(self, client: AsyncClient, manager_token):
        r = await client.get(
            "/api/v1/reports/recent-decisions",
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        assert r.status_code == 200
        assert isinstance(r.json(), list)
...
class TestPromotionReady:
    async def test_promotion_ready(
        self, client: AsyncClient, manager_token, employee_user, skill
    ):
        r = await client.get(
            "/api/v1/reports/promotion-ready",
            headers={"Authorization": f"Bearer {manager_token}"},
        )
        assert r.status_code == 200
        assert isinstance(r.json(), list)
